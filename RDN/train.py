...
import RDN.option as option
from RDN.utils import SaveData
from RDN.data import MDE_Dataset
from RDN.model import rdn_denoise
...
def main(args, start_time, set_loss, lr_scheduler):
    ...
    train_loader = torch.utils.data.DataLoader(
        data_train, batch_size=args.batch_size, drop_last=True, shuffle=True, num_workers=32, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(
        data_val, batch_size=args.batch_size, drop_last=False, shuffle=True, num_workers=32, pin_memory=True)
    ...
    # Resume training
    if args.resume:
        model_dir = os.path.join(*savedata.checkpoint.split("/")[:-1])
        time_dir = sorted(os.listdir(model_dir))[-2]
        model_data = torch.load("{}/{}/model/best_model_val.pt".format(model_dir, time_dir))
        optimizer_path = "{}/{}/optimizer/best_model_val.pt".format(model_dir, time_dir)
    ...
    with tqdm(total=max_epoch) as pbar:
        for i in range(max_epoch):
            ...
            with tqdm(total=len(train_loader), leave=False) as pbar_train:
                for im, im_gt, _, _ in train_loader:
            ...
            with torch.no_grad():
                with tqdm(total=len(val_loader), leave=False) as pbar_val:
                    for im, im_gt, _, _ in val_loader:
            ...
            if bestscore_train > epoch_loss:
                ...
                savedata.save_model(model, optimizer, 'best_model_train.pt')
            ...
            if bestscore_val > epoch_loss_val:
                ...
                savedata.save_model(model, optimizer, 'best_model_val.pt')
